...
```c++
template <typename T, std::size_t N> class BoundedArray;
```
...
using BoundedArray::size_type = std::size_t;
...
```c++
static constexpr std::size_t BoundedArray::bound = N;
```
...
explicit BoundedArray::BoundedArray(std::size_t n, const T& t = {});
...
```c++
T& BoundedArray::operator[](std::size_t i) noexcept;
const T& BoundedArray::operator[](std::size_t i) const noexcept;
T& BoundedArray::at(std::size_t i);
const T& BoundedArray::at(std::size_t i) const;
...
```c++
std::size_t BoundedArray::capacity() const noexcept;
void BoundedArray::clear() noexcept;
bool BoundedArray::empty() const noexcept;
void BoundedArray::resize(std::size_t n, const T& t = {});
std::size_t BoundedArray::size() const noexcept;
```
...
```c++
std::size_t BoundedArray::hash() const noexcept;
...
```c++
std::strong_ordering operator<=>(const BoundedArray& a,
    const BoundedArray& b) noexcept;
