...
```c++
template <typename T, std::size_t N> class CompactArray
```
...
using CompactArray::size_type = std::size_t;
...
```c++
static constexpr std::size_t CompactArray::threshold = N;
```
...
explicit CompactArray::CompactArray(std::size_t n, const T& t = {});
...
```c++
T& CompactArray::operator[](std::size_t i) noexcept;
const T& CompactArray::operator[](std::size_t i) const noexcept;
T& CompactArray::at(std::size_t i);
const T& CompactArray::at(std::size_t i) const;
...
```c++
std::size_t CompactArray::capacity() const noexcept;
void CompactArray::clear() noexcept;
bool CompactArray::empty() const noexcept;
bool CompactArray::is_compact() const noexcept;
void CompactArray::reserve(std::size_t n);
void CompactArray::resize(std::size_t n, const T& t = {});
void CompactArray::shrink_to_fit();
std::size_t CompactArray::size() const noexcept;
```
...
```c++
std::size_t CompactArray::hash() const noexcept;
...
```c++
std::strong_ordering operator<=>(const CompactArray& a,
    const CompactArray& b) noexcept;
