#include "rs-containers/spatial-index.hpp"
...
namespace {
    ...
    std::string dump(const Index& index) {
        std::vector<std::string> ss;
        for (auto [k,v]: index) {
            ss.push_back(std::format("{0} {1}\n", k, v));
        }
        std::ranges::sort(ss);
        return std::accumulate(ss.begin(), ss.end(), std::string{});
    }
...
void test_rs_containers_spatial_index_insert() {
    ...
    TEST_THROW(space.insert("zulu", {1e50, 0}), std::out_of_range, "entry is out of range");
...
void test_rs_containers_spatial_index_search() {
    ...
    auto hits = [&its] () {
        std::string str;
        std::transform(its.begin(), its.end(), std::back_inserter(str), [] (auto& it) { return it->first[0]; });
