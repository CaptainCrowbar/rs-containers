...
namespace RS::Containers {
    ...
    public:
        ...
        class iterator:
        public Iterator<iterator, const value_type, std::forward_iterator_tag> {
        public:
            iterator() = default;
        ...
        std::size_t size() const noexcept { return obj_cell_map_.size(); }
    ...
    private:
        ...
        SpatialIndex<T, S, N>::search(vector_type origin, S radius) const {
            ...
            if (origin_r2 <= r2_limit_ && radius <= outer_limit_) {
                ...
                std::vector<index_type> linear_offsets(std::size_t(2 * max_offset + 1));
